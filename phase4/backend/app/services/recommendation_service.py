...
class RecommendationService:
    ...
    - Sort by rating (descending), with a small boost for restaurants the user liked before.
    ...
    async def get_recommendations(
        ...
        # Load all restaurants from the database.
        all_restaurants = self._restaurant_repo.get_all()

        # Determine which restaurants this user has liked before.
        liked_ids = set()
        if preferences.user_id:
            liked_ids = self._feedback_repo.get_liked_restaurant_ids(preferences.user_id)
    ...
    def _matches_location(self, restaurant: dict, preferences: RecommendationRequest) -> bool:
        ...
        # The underlying dataset only contains specific Bangalore neighborhoods.
        # If the user searches for "Bangalore" broadly, we can match all of them.
    ...
    def _matches_min_rating(self, restaurant: dict, preferences: RecommendationRequest) -> bool:
        ...
        if rating is None:
            return False
        # Broaden the search by applying a 0.5 tolerance to the minimum rating
